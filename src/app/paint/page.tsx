...
export default async function PaintDashboard({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <>
    <div className="row"><h1>Paint production tracking</h1><Link className="button" href="/paint/runs/new">Create paint run</Link></div>
    <form className="card row">
      ...
      <button>Filter</button><Link className="button ghost" href="/paint">Reset</Link>
    </form>
    <div className="card"><table><thead><tr><th>Run</th><th>Status</th><th>Planned</th><th>Loaded</th><th>Sprayed</th><th>Unloaded</th><th>RFT OK</th><th>Reject</th><th>RFT %</th><th>Actions</th></tr></thead><tbody>{runs.map(r=><tr key={r.id}><td><Link href={`/paint/runs/${r.id}`}>{r.run_number}</Link></td><td><span className="badge">{r.status}</span></td><td>{r.planned_date ?? "—"}</td><td>{r.total_loaded_qty}</td><td>{r.total_sprayed_qty}</td><td>{r.total_unloaded_qty}</td><td>{r.total_rft_ok_qty}</td><td>{r.total_reject_qty}</td><td>{r.rft_percentage == null ? "—" : `${r.rft_percentage}%`}</td><td className="actions">{r.status === "LOADED" && <Link href={`/paint/runs/${r.id}/spraying`}>Spray</Link>}{r.status === "SPRAYED" && <Link href={`/paint/runs/${r.id}/unloading`}>Unload</Link>}</td></tr>)}</tbody></table>{runs.length===0 && <p className="muted">No paint runs match the current filters.</p>}</div>
  </>;
}
